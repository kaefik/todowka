# L8-59 — Создать README.md с инструкциями по запуску

## Цель
Документировать как запускать приложение.

## Вход
Все предыдущая работа.

## Выход
README.md с инструкциями по setup и запуску.

## Готово когда
README включает: описание проекта, tech stack, шаги установки, как запускать API, Redis, Celery worker и beat, документацию API.

## Подсказка для LLM
Создайте README.md с секциями: Описание Проекта, Tech Stack, Установка (pip install -r requirements.txt), Настройка Окружения (скопируйте .env.example в .env), Запуск Приложения (команды для uvicorn, redis-server, celery worker, celery beat), Документация API (http://localhost:8000/docs), Структура Проекта, Будущие Расширения.
...
#### Added
- Initial release
- CRUD для задач, проектов, подзадач
- GTD поддержка (Inbox, Next Actions, Contexts, Areas)
- Теги и пагинация
