# L2-35 — Создать ContextService (CRUD)

## Цель
Определить ContextService с бизнес-логикой.

## Вход
Context репозиторий (L1-28), Context схемы (L1-20).

## Выход
app/services/context.py.

## Готово когда
ContextService с методами CRUD.

## Подсказка для LLM
Создайте app/services/context.py с классом ContextService. Методы: __init__(self, context_repo: ContextRepository), create(self, name: str, icon: Optional[str] = None, color: Optional[str] = None) -> ContextResponse, get_all(self) -> List[ContextResponse], get_by_id(self, id: int) -> ContextResponse (бросает NotFoundException), update(self, id: int, **kwargs) -> ContextResponse, delete(self, id: int).
